...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  const serializedOptions = JSON.stringify({
    ...
    lanesPerRoad: effectOptions?.lanesPerRoad,
    totalSideLightSticks: effectOptions?.totalSideLightSticks,
    lightPairsPerRoadWay: effectOptions?.lightPairsPerRoadWay,
  ...
  useEffect(() => {
    ...
    class App {
      ...
      constructor(container, options: any = {}) {
        ...
        // Safety guard against postprocessing/three.js getContextAttributes() returning null error
        const originalGetContext = this.renderer.getContext.bind(this.renderer);
        this.renderer.getContext = () => {
          const context = originalGetContext();
          if (context && !(context as any).__contextAttributesWrapped) {
            (context as any).__contextAttributesWrapped = true;
            const originalGetContextAttributes = context.getContextAttributes ? context.getContextAttributes.bind(context) : null;
            context.getContextAttributes = () => {
              const attrs = originalGetContextAttributes ? originalGetContextAttributes() : null;
              return attrs || {
                alpha: true,
                antialias: false,
                depth: true,
      ...
      initPasses() {
        ...
        const isLightBg = options.colors && options.colors.background > 0x888888;
        this.bloomPass = new EffectPass(
          this.camera,
          new BloomEffect({
            luminanceThreshold: isLightBg ? 0.95 : 0.2,
            luminanceSmoothing: isLightBg ? 0.05 : 0,
            intensity: isLightBg ? 0.4 : 1.0,
            resolutionScale: 1
          })
        );
      ...
      loadAssets() {
        ...
        return new Promise(resolve => {
          const manager = new THREE.LoadingManager(() => resolve(null));
      ...
      update(delta) {
        let speedCoeff = 0.7; // Reduce animation speed by 30%
        let modifiedDelta = delta * speedCoeff;
        let lerpPercentage = Math.exp(-(-60 * Math.log2(1 - 0.1)) * modifiedDelta);
        this.speedUp += lerp(this.speedUp, this.speedUpTarget, lerpPercentage, 0.00001);
        this.timeOffset += this.speedUp * modifiedDelta;

        this.customTime += modifiedDelta;
        let time = this.customTime + this.timeOffset;
